...
function parseWhois(domain: string): DomainReport["whois"] {
  ...
  // Expiry date
  const expiryMatch = whoisOutput.match(/Registry Expiry Date:\s*(.+)/i) || whoisOutput.match(/Expiration Date:\s*(.+)/i);
...
function parseSecurity(domain: string, dns: DomainReport["dns"]): DomainReport["security"] {
  ...
  const selectors = ["default", "google", "k1", "selector1", "selector2", "zmail", "cf2024-1"];
